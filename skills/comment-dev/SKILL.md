...
name: comment-dev
description: >-
  The front door for engineering work through a Comment.io worklog. Talk to it
  about dev work in plain language and it picks the right path: shape a rough
...
| A defined feature; "build / add / implement / wire up X", clear what success is | **`comment-feature`** | Full plan → build → review → ship through a worklog. |
...
- **Identity / worklog.** The chosen skill opens the worklog and (via
  `comment-identity`) the session-scoped ethereal handle. If the user already
...
read **`AGENTS.md` (else `CLAUDE.md`)** and its linked **`docs/TESTING.md`**
(test lanes), PR/deploy docs, and architecture. If those are missing, the routed
skill infers them or offers to run **`comment-init`** to scaffold them.
