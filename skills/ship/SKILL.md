...
1. **Pre-flight**: run the repo's **`full`** lane (see **Repo config**; `docs/TESTING.md`) on the **staged/uncommitted** diff **first** (AGENTS.md: always run tests before committing); skip only for genuinely docs-only changes. Once they pass, commit so the tree is clean (`git status` clean) — don't push or review the wrong HEAD. Record results in the worklog; if a Project Root exists, summarize only pass/fail and blockers there.
2. **Review gate — Codex path (local, before push)**: if `codex` is available, run it on the local diff. For each real finding: fix it → **re-run step 1's quality gates** (a review fix is new code and must pass the gates before it's pushed) → re-commit → re-run the reviewer. Loop until clean. (No Codex → the review happens at step 4 on the PR.)
3. **Push**: push the committed branch whenever local is ahead of remote — this covers both the first push **and** resuming an existing PR after local fixes (`git push` / `git push -u origin HEAD`). Skip entirely if the user said not to push a PR.
...
5. **Merge-ready loop (the "pristine" gate — default stopping point).** Loop until every criterion holds; report "merge-ready" when it does (don't merge unless explicitly asked). Re-confirm CI **and** mergeability at the moment you exit — both can drift while you wait.
   - **CI**: `gh pr checks <n> --required`. Required jobs must all be `pass` or `skipping`. If any fail, read `gh run view --log-failed`, fix → back to step 1 (gates) → commit → push, and re-enter the loop. If pending, wait.
   ...
   - **Review threads**: address every unresolved, non-outdated thread — fix → commit → push (back through steps 1–3) → reply with what changed → resolve; or, if you disagree, reply with concise reasoning and leave it (a real unresolved disagreement is not merge-ready). Re-check after each new head.
